...
export default function ProviderDashboard() {
  ...
  const generateMotto = () => {
    const adjs = ['Elite', 'Seamless', 'Precision', 'Paramount', 'Aesthetic'];
    const verbs = ['Architecting', 'Curating', 'Redefining', 'Pioneering'];
    const nouns = ['Excellence', 'Infrastructure', 'Experiences', 'Dominance'];

    const motto = `${verbs[Math.floor(Math.random() * verbs.length)]} ${
      adjs[Math.floor(Math.random() * adjs.length)]
    } ${nouns[Math.floor(Math.random() * nouns.length)]} in Nairobi.`;

    const bioInput = document.querySelector('textarea[name="bio"]') as HTMLTextAreaElement;
    if (bioInput) bioInput.value = motto;
  };
  ...
  return (
    <main className="min-h-screen bg-[#F5F5F7] p-8 font-sans">
      <div className="max-w-xl mx-auto bg-white rounded-[2rem] p-10 shadow-xl border border-gray-100">
        ...
        <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-400 mb-10">
        ...
        <form onSubmit={handleUpdate} className="space-y-6">
          <div>
            <label className="text-[10px] font-black uppercase text-gray-400 block mb-2">
              Your Node ID (Required)
            </label>
            <input
              name="nodeId"
              required
              className="w-full bg-gray-50 border-none rounded-xl p-4 font-mono font-bold"
              placeholder="e.g. 7"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              name="bio"
              rows={4}
              className="w-full bg-gray-50 border-none rounded-xl p-4 text-sm outline-none focus:ring-1 focus:ring-black"
            />
          </div>
          ...
          <button
            ...
            className="text-[10px] font-black uppercase text-gray-400 hover:text-black mb-4 flex items-center gap-2"
          >
